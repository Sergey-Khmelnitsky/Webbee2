...
class SlotGeneratorService
{
    ...
    private function findCommonSlotsForInstances(array $serviceSlots, array $serviceConfigs, Carbon $date): array
    {
        ...
        // If only one service instance, return its slots directly
        if (count($serviceSlots) === 1) {
            return $serviceSlots[0];
        }

        // Check if all service instances are for the same service
        // If so, slots should be identical, so we can just return the first instance's slots
        $firstServiceId = $serviceConfigs[0]['service_id'];
        $allSameService = true;
        foreach ($serviceConfigs as $config) {
            if ($config['service_id'] != $firstServiceId) {
        ...
        if ($allSameService) {
            // All instances are for the same service, so slots should be identical
            // Return all slots from the first instance
            return $serviceSlots[0];
        }
